...
describe('Timer', function(){
  ...
  describe('TimeLimit', function(){
    it('should accept an argument of maximum time, default to 10', function(){
      timer.start();
    ...
    it('should have 20 tick events triggered with an argument of 20', function(){
      var timer20 = new BasicTimer(20);
      timer20.on('tick', tickHandler);
      timer20.start();
    ...
    it('should emit a "complete" event when the final tick is completed', function(){
      timer.start();
      this.clock.tick(10000);
      expect(tickHandler.callCount).to.equal(10);
      this.clock.tick(1000);
      ...
      var timeCallback = sinon.spy();
      timeCallback({totalTime: 10000});
      timer.on('complete', timeCallback);
      expect(timeCallback.calledWithMatch({totalTime: 10000})).to.be.true;

      // timer.on('complete', function (event){
      //   expect(event.totalTime).to.be.above(10000);
      //   done();
      // });
    });
  });
